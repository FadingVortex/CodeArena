...
// "http://localhost:8001/think/query/questions?LMC=L2-2-1"
export const getQueryQuestions = (opt) => {
    let arr1 = opt.url.split('?');  // 两个元素，第二个元素是参数
    let arr2 = arr1[1].split('=');  
    let LMC = arr2[1];
    let x = TestDB.filter(el => el.LMC.startsWith(LMC));
    return {
        code: 200,
        msg: '提取题库成功',
        data: {
...
export const deleteQuestions = (opt) => {
    let idsObj = JSON.parse(opt.body);
    let {ids} = idsObj;
    console.log(ids);
    console.log(ids instanceof Array);
    ids.forEach(id => {
        for (let i = TestDB.length - 1; i >= 0; i--) {
            if (TestDB[i].id === id) {
                TestDB.splice(i, 1);  // 删除元素
...
export const getQueryPageQuestions = (opt) => {
    let {LMC, currentPage, pageSize} = JSON.parse(opt.body);
    // 满足查询条件的数据
    let findData = [];

    if(LMC === undefined || LMC.length <= 0){
        findData = TestDB;
    } else {
        findData = TestDB.filter(el => el.LMC.startsWith(LMC));
    }
